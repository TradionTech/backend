...
import { RiskCalculation } from '../db/models/RiskCalculation.js';
...
export const riskController = {
  calculate: async (req: Request, res: Response) => {
    ...
    if ((entry > stop_loss && take_profit && take_profit <= entry) ||
    ...
    const risk_reward_ratio = take_profit ? Math.abs((take_profit - entry) / (entry - stop_loss)) : null;
